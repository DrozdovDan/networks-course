...
#### Решение
$T = \dfrac{W}{2}RTT$. Среднее значение размера окна $\overline{W}=\dfrac{3W}{4}$. 
Средняя пропускная способность $B = \dfrac{\overline{W}}{RTT} = \dfrac{3W}{4RTT}$, тогда 
$W = \dfrac{4B}{3}RTT$. Тогда $T = \dfrac{2B}{3}RTT$. Ч.Т.Д.
...
#### Решение
У нас размеры сегментов будут: 1, 2, 4, 8 в каждом раунде передачи соответственно. 
Тогда время передачи для одного раунда будет $Td_i = \max(RTT, 2^{i-1}\dfrac{S}{R})$. Время установления 
соединения $=RTT$. Значит, суммарное время одного раунда $T_i = RTT$, if $2^{i - 1}\dfrac{S}{R} \leq RTT$, else 
$2^{i - 1}\dfrac{S}{R} + RTT$. 
Таким образом суммарное время передачи займёт $RTT + \sum_{i=1}^4T_i$.
1. Из неравенств следует, что время первого раунда будет $RTT$, остальных — 
$\sum_{i=2}^4 RTT + 2^{i-1}\dfrac{S}{R} = 14\dfrac{S}{R} + 3RTT$, тогда итоговое время передачи $T=5RTT+14\dfrac{S}{R}$.
2. Здесь все передачи медленнее $RTT$, значит, $T=5RTT$. 
3. Здесь наоборот, значит, $T=5RTT + 15\dfrac{S}{R}$
...
#### Решение
В таком алгоритме у нас повышение от $\dfrac{W}{2}$ до $W$ занимает $n$ шагов, при этом 
$W=\dfrac{W}{2}(1+a)^n$, тогда $(1+a)^n=2$. Сегментов на каждом шаге $w_i = \dfrac{W}{2}(1+a)^i$. 
За цикл передается $D = \sum_{i=0}^{n-1}w_i = \dfrac{W}{2}\dfrac{(1+a)^n-1}{1+a-1}=\dfrac{W}{2a}$. 
Потеря происходит в раз за цикл, поэтому $L = \dfrac{1}{D} = \dfrac{2a}{W}$.
...
#### Решение
На общение с сервером необходимо в точности $4RTT_{FE}$, между собой у серверов 
обмен данными происходит за $RTT_{BE}$. Добавляем обработку данных, получается, 
время отклика равно $4RTT_{FE} + RTT_{BE} + \text{время обработки}$, ч.т.д.
