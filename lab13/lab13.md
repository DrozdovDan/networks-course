...
3. Каковы значения Transaction-ID в наборе (Request/ACK) DHCP-сообщений? Опишите
   ...
   - 0x2170d6c8
   - Transaction-ID используется для идентификации ответов и запросов поступающих от одной сессии пользователя, когда 
   у нас нет ip в сессии и производится широковещательная рассылка.
...
6. Объясните назначение срока аренды IP-адреса. Какова длительность срока аренды в
   ...
   - IP Address Lease Time: 2 hours (7200). Он нужен, чтобы забирать у клиентов, отключившихся от сети, ip-address и 
   отдавать его новым клиентам.
...
#### Решение
1. $\dfrac{\partial}{\partial p}N p (1 - p) ^ {N - 1} = N((1 - p)^{N - 1} - (N - 1) p (1 - p)^{N - 2}) = 0 
\Rightarrow p (N - 1) = 1 - p \Rightarrow p = \dfrac{1}{N}$
2. $\lim_{N \rightarrow \infty} N \dfrac{1}{N} (1 - \dfrac{1}{N})^{N-1} = 
\lim_{N \rightarrow \infty} (1 - \dfrac{1}{N}) ^ N = e^{-1}$
...
#### Решение
1. Один узел передает, остальные три не передают. Вероятность этого $p (1 - p)^3$.
2. Один из трех узлов передает, остальные три не передают. Вероятность этого $3 p (1 - p)^3$.
3. Вероятность успешной передачи за квант $4 p (1 - p)^3$, значит, вероятность не передачи
1 - 4 p (1 - p)^3. Тогда искомая вероятность 4 p (1 - p)^3 (1 - 4 p (1 - p)^3)^2.
4. $4 p (1 - p)^3$.
...
#### Решение
Все время можно разбить на сессию, в которой один из $N$ узлов отвечает. Тогда $Q$ бит 
информации будет передано за $d_{\text{опрос}} + \dfrac{Q}{R}$. В таком случае максимальная 
пропускная способность равна $\dfrac{Q}{d_{\text{опрос}} + \frac{Q}{R}}$.
